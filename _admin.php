    ...
    public static function pageForm(&$post) {
        ...
        while ($i==0 || $rs->fetch()) {
            ...
            $rep.='<div class="area" id="translation-'.$i.'-area"><label id="translation-'.$i.'-langlabel" class="classic">'.($i?__('Translation:'):__('New translation:')).
                form::field('post_translation_'.$i.'_lang',3,255,html::escapeHTML($t_lang),'',5*$i+14).
                '</label> '.
                '<label class="classic">'.__('Translation title:').
                form::field('post_translation_'.$i.'_title',40,255,html::escapeHTML($t_title),'',5*$i+15).
                ...
                form::textarea('post_translation_'.$i,50,5,html::escapeHTML($t_content),'',5*$i+17).
                '<div class="lockable">'.
                '<p><label>'.__('Basename:').
                form::field('post_translation_'.$i.'_url',60,255,html::escapeHTML($t_url),'',5*$i+18).
                '</label></p>'.
